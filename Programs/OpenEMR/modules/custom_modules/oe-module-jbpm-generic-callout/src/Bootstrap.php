...
class Bootstrap
{
    ...
    const MODULE_NAME = "oe-module-jbpm-generic-callout";
    /**
    ...
    public function addGlobalSettingsSection(GlobalsInitializedEvent $event)
    {
        ...
        $section = xlt("jBPM Generic Callout");
    ...
    {
        ?>
        <link rel="stylesheet" href="<?php echo $this->getAssetPath();?>css/jbpm-generic-callout.css">
        <script src="<?php echo $this->getAssetPath();?>js/jbpm-generic-callout.js"></script>
        <?php
    }
    ...
    public function addCustomModuleMenuItem(MenuEvent $event)
    {
        ...
        $menuItem->label = xlt("jBPM Generic Callout");
        ...
        $menuItem->url = "/interface/modules/custom_modules/oe-module-jbpm-generic-callout/public/sample-index.php";
        $menuItem->children = [];
         ...
         */
        //$menuItem->global_req = ["jbpm_generic_callout_enable"];
    ...
    public function subscribeToApiEvents()
    {
        if ($this->getGlobalConfig()->getGlobalSetting(GlobalConfig::CONFIG_ENABLE_FHIR_API)) {
            $this->eventDispatcher->addListener(RestApiCreateEvent::EVENT_HANDLE, [$this, 'addjBPMGenericCalloutApi']);
    ...
    public function addjBPMGenericCalloutApi(RestApiCreateEvent $event)
    {
        $apiController = new jBPMGenericCalloutRestController();

        /**
         * To see the route definitions @see https://github.com/openemr/openemr/blob/master/_rest_routes.inc.php
         */
        $event->addToFHIRRouteMap('GET /fhir/jBPMGenericCalloutResource', [$apiController, 'listResources']);
        $event->addToFHIRRouteMap('GET /fhir/jBPMGenericCalloutResource/:fhirId', [$apiController, 'getOneResource']);
    ...
    public function addApiScope(RestApiScopeEvent $event)
    {
        if ($event->getApiType() == RestApiScopeEvent::API_TYPE_FHIR) {
            ...
            $scopes[] = 'user/jBPMGenericCalloutResource.read';
            $scopes[] = 'patient/jBPMGenericCalloutResource.read';
            // only add system scopes if they are actually enabled
            if (\RestConfig::areSystemScopesEnabled())
            {
                $scopes[] = 'system/jBPMGenericCalloutResource.read';
            }
    ...
    {
        $event->setServiceClass(jBPMGenericCalloutFHIRResourceService::class);
        return $event;
    }
