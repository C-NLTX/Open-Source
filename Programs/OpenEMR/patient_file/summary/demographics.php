...
<body class="mt-1 patient-demographic bg-light">
	...
	<?php
	$response = "";
	//$content_prediction00 = "";
	if(isset($_POST['predict00']))
	{
		$ch = curl_init();
		curl_setopt($ch, CURLOPT_POST, true);
		curl_setopt($ch, CURLOPT_HTTPAUTH, CURLAUTH_ANY);
		curl_setopt($ch, CURLOPT_USERPWD, "wbadmin:wbadmin");
		curl_setopt($ch, CURLOPT_URL, "http://localhost:8080/kie-server/services/rest/server/containers/project_1.0.0-SNAPSHOT/processes/stroke-act/instances");
		...
		$lockData = "";
		file_put_contents($lockname, $lockData);
		
		$filename = "./stroke.txt";
		$stroke_params = file($filename);
	
		$stroke_sql = "SELECT id, sex AS gender, usertext1 AS age, usertext2 AS hypertension, usertext3 AS heart_disease, family_size AS ever_married, occupation AS work_type, usertext4 AS residence_type, usertext5 AS avg_glucose_level, usertext6 AS bmi, usertext7 AS smoking_status, usertext8 AS stroke FROM `patient_data` WHERE id = ?";
		$stroke_res = sqlStatement($stroke_sql, [$pid]);
		$stroke_row = sqlFetchArray($stroke_res);
		$stroke_id=$stroke_row['id'];
		$stroke_gender=$stroke_row['gender'];
		$stroke_age=$stroke_row['age'];
		$stroke_hypertension=$stroke_row['hypertension'];
		$stroke_heart_disease=$stroke_row['heart_disease'];
		$stroke_ever_married=$stroke_row['ever_married'];
		$stroke_work_type=$stroke_row['work_type'];
		$stroke_residence_type=$stroke_row['residence_type'];
		$stroke_avg_glucose_level=$stroke_row['avg_glucose_level'];
		$stroke_bmi=$stroke_row['bmi'];
		$stroke_smoking_status=$stroke_row['smoking_status'];
		$stroke_stroke=$stroke_row['stroke'];

		$newData = $stroke_params[0].
		$stroke_params[1].
		$stroke_params[2].
		$stroke_params[3].
		$stroke_params[4].
		...
		$filename = "./stroke.txt";
		$stroke_params = file($filename); 
		curl_setopt($ch, CURLOPT_POSTFIELDS, "{ \"iterations\": { \"com.discovery.project.Iterations\": { \"number\": $stroke_params[0] } },".
		" \"workdir\": { \"com.discovery.project.Workdir\": { \"path\": $stroke_params[1] } },".
		" \"dropcols\": { \"com.discovery.project.Dropcols\": { \"droplist\": $stroke_params[2] } },".
		" \"samplestruct\": { \"com.discovery.project.Samplestruct\": { \"balancing\": $stroke_params[3] } },".
		" \"act\": { \"com.discovery.project.Act\": { \"action\": $stroke_params[4] } },".
		" \"train\": { \"com.discovery.project.Train\": { \"folds\": $stroke_params[6], \"bagfrac\": $stroke_params[7] } },".
		" \"test\": { \"com.discovery.project.Test\": { \"predfrac\": $stroke_params[5] } },".
		" \"record\": { \"com.discovery.project.Record\": { \"id\": $stroke_params[8], \"gender\": $stroke_params[9], \"age\": $stroke_params[10], \"hypertension\": $stroke_params[11], \"heart_disease\": $stroke_params[12], \"ever_married\": $stroke_params[13], \"work_type\": $stroke_params[14], \"residence_type\": $stroke_params[15], \"avg_glucose_level\": $stroke_params[16], \"bmi\": $stroke_params[17], \"smoking_status\": $stroke_params[18], \"stroke\": $stroke_params[19] } } }");
		$stroke_result = curl_exec ($ch);
		$response = json_decode($stroke_result);
		...
		$content_stroke=$content['variable-instance'][28]['value'];
		$content_prediction00=$content['variable-instance'][30]['value'];
		curl_close($ch);
	...
	<form method="post" action="demographics.php">
		<input type="submit" name="predict00" value="Stroke Risk">
	</form>
	<ul>
		<li>Response ID: <?php echo $response; ?></li>
		<li>Response code: <?php echo $status_code; ?></li>
		<li>Known risk: <?php echo $content_stroke; ?></li>
		<li>Predicted risk: <?php echo $content_prediction00; ?></li>
	</ul>
	<?php
	...
	<form method="post" action="demographics.php">		
		<input type="text" name="number" id="number" value="<?php echo $stroke_params[0]; ?>">
		<label for="number">- number of loop iterations</label>
		<br><input type="text" name="path" id="path" value='<?php echo $stroke_params[1]; ?>'>
		<label for="path">- path to working directory</label>
		<br><input type="list" name="droplist" id="droplist" value="&quot;['id','gender','ever_married','work_type','residence_type','smoking_status','stroke']&quot;">
		<label for="droplist">- list of columns to ignore in models</label>
		<br><input type="text" name="balancing" id="balancing" value="<?php echo $stroke_params[3]; ?>">
		<label for="balancing">- sample balancing ratio</label>
		<br><input type="text" name="action" id="action" value="<?php echo $stroke_params[4]; ?>">
		<label for="action">- number of action runs</label>
		<br><input type="text" name="predfrac" id="predfrac" value="<?php echo $stroke_params[5]; ?>">
		<label for="predfrac">- sample training fraction</label>
		<br><input type="text" name="folds" id="folds" value="<?php echo $stroke_params[6]; ?>">
		<label for="folds">- number of folds</label>
		<br><input type="text" name="bagfrac" id="bagfrac" value="<?php echo $stroke_params[7]; ?>">
		<label for="bagfrac">- training sample bagging fraction</label>
		<br><input type="text" name="id" id="id" value="<?php echo $stroke_id; ?>">
		<label for="id">- record: id</label>
		<br><input type="text" name="gender" id="gender" value="&quot;<?php echo $stroke_gender; ?>&quot;">
		<label for="gender">- record: gender</label>
		<br><input type="text" name="age" id="age" value="<?php echo $stroke_age; ?>">
		<label for="age">- record: age</label>
		<br><input type="text" name="hypertension" id="hypertension" value="<?php echo $stroke_hypertension; ?>">
		<label for="hypertension">- record: hypertension</label>
		<br><input type="text" name="heart_disease" id="heart_disease" value="<?php echo $stroke_heart_disease; ?>">
		<label for="heart_disease">- record: heart_disease</label>
		<br><input type="text" name="ever_married" id="ever_married" value="&quot;<?php echo $stroke_ever_married; ?>&quot;">
		<label for="ever_married">- record: ever_married</label>
		<br><input type="text" name="work_type" id="work_type" value="&quot;<?php echo $stroke_work_type; ?>&quot;">
		<label for="work_type">- record: work_type</label>
		<br><input type="text" name="residence_type" id="residence_type" value="&quot;<?php echo $stroke_residence_type; ?>&quot;">
		<label for="residence_type">- record: residence_type</label>
		<br><input type="text" name="avg_glucose_level" id="avg_glucose_level" value="<?php echo $stroke_avg_glucose_level; ?>">
		<label for="avg_glucose_level">- record: avg_glucose_level</label>
		<br><input type="text" name="bmi" id="bmi" value="<?php echo $stroke_bmi; ?>">
		<label for="bmi">- record: bmi</label>
		<br><input type="text" name="smoking_status" id="smoking_status" value="&quot;<?php echo $stroke_smoking_status; ?>&quot;">
		<label for="smoking_status">- record: smoking_status</label>
		<br><input type="text" name="stroke" id="stroke" value="<?php echo $stroke_stroke; ?>">
		<label for="stroke">- record: stroke</label>
